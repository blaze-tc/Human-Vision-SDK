param([string]$Filter = '', [string]$VisualStudio = 'D:/Microsoft Visual Studio')
...
"$cmake" -S "$taskRoot" -B "$taskRoot/build/windows-test" -G "Ninja Multi-Config" -DBUILD_TESTING=ON -DHV_USE_DIRECTML=OFF
if errorlevel 1 exit /b 1
"$cmake" --build "$taskRoot/build/windows-test" --config Release
if errorlevel 1 exit /b 1
