...
std::uint8_t BilinearRgb(
    ...
    const auto top_left = ReadRgb(frame, x0, y0);
    const auto top_right = ReadRgb(frame, x1, y0);
    const auto bottom_left = ReadRgb(frame, x0, y1);
    const auto bottom_right = ReadRgb(frame, x1, y1);
        ...
        for (int x = 0; x < kInputWidth; ++x) {
            ...
            for (int channel = 0; channel < 3; ++channel) {
                const std::uint8_t value =
                    BilinearRgb(frame, source.x, source.y, channel);
