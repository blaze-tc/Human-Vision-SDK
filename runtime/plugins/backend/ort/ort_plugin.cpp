...
namespace {
struct Session {
 explicit Session(bool accelerated):backend(accelerated),requested(accelerated?"accelerated":"CPU"){}
...
HV_Result CreateSession(const HV_BackendConfigV1* config,void** out,HV_ErrorBufferV1* error,bool accelerated) {
...
#endif
 if(config->requested_provider_utf8 && *config->requested_provider_utf8 && std::strcmp(config->requested_provider_utf8,expected) && !( !accelerated && !std::strcmp(config->requested_provider_utf8,"cpu"))) {
  Error(error,"Backend plugin cannot satisfy the requested provider");return HV_ERR_INVALID_ARGUMENT;
 }
 try {
  auto session=std::make_unique<Session>(accelerated);session->requested=expected;std::string detail;
...
HV_Result HV_CALL Info(void* p,HV_BackendSessionInfoV1* out){
  ...
  *out={};out->struct_size=sizeof(*out);out->api_version=HV_PLUGIN_API_V1;
  ...
  out->accelerated=actual=="NNAPI" || actual=="DirectML";return HV_OK;
