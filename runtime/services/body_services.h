...
enum class BodySampleState { Predicted, Held, Stale };
// Internal telemetry only; the public canonical body and C ABI remain unchanged.
struct BodySampleDiagnostics {
 double observation_period_ewma_ms=0,render_hold_ms=500,track_lost_ms=800,sample_age_ms=-1;
 uint32_t tracked_body_count=0,sampled_body_count=0;
 BodySampleState state=BodySampleState::Stale;
};
...
public:
 ...
 void Observe(const HV_ObservationFrameV1&,int64_t revision,bool preserve_previous_hands=true,
              const int32_t* assigned_regions=nullptr,const int32_t* crop_track_ids=nullptr);
 TrackAnchors Anchors() const;
 ...
 BodySampleDiagnostics Diagnostics(int64_t now_us) const;
 uint32_t HandRequests(HV_RegionOfInterestV1*,uint32_t capacity,int64_t now_us,int64_t minimum_interval_us=0);
private:
 struct Track {HV_CanonicalBodyV1 raw{},filtered{};std::array<float,32> vx{},vy{};HV_Rect box_velocity{};std::array<int64_t,2> hand_request_time{};int crop_track_id=0,hits=0;bool active=false;};
 ...
 double observation_period_ewma_us_=0;
 int64_t RenderHoldUs() const;
 int64_t TrackLostUs() const;
